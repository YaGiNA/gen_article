...
\begin{abstract}
Recently, fake news is shared via social networks and makes false rumors more diffusible.
...
It is confirmed that considering not only articles but also social contexts(i.e. likes, retweets, replies, comments) supports to spot fake news correctly by them.
Likewise, since the social contexts are also made by sharing on the social network, it is hard to detect fake news earlier.
We propose a fake news detector with the ability to generate fake social contexts, aiming to detect such fake news as to spread only if accompanied by a certain context. The fake context generation is based on a fake news generator model. 
This model is trained about generating comments by dataset which is combined news and comments.
In addition, we also trained the model in classifying news and comments which include generated one into real and fake.
To measure this model's detection quality, we checked classification results from articles with real comments and generated ones by itself.
As a result, we conclude that considering a generated comment supports detect more fake news than considering real comments only.
It suggests that our proposed model will help to spot fake news on social networks.
\end{abstract}
...
Directer of General of the WHO called this problem ``infodemic'' and he told that fake news shares faster and more easily than itself\cite{ZAROCOSTAS2020676}. 
