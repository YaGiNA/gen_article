...
We propose a fake news detector with the ability to generate fake social contexts, aiming to detect fake news in early stage of its diffusion where few social contexts are available.
...
This used news article, real-posted comments, and generated comments.
To measure our detector’s effectiveness, we examined performance of the generated comments for articles with real comments and generated ones by the classify model.
...
Some of them have misinformation that is made by malicious purpose.
...
Therefore, Two-Level Convolutional Neural Network with User Response Generator(TCNN-URG) was proposed\cite{ijcai2018-533}.
...
However, the TCNN-URG generates only words that have a high probability of appearance in a comment and there are no grammar elements.
...
We modeled generation model by the joint distribution like the original one:
\begin{equation}
...
The last token of integrated news fields were \texttt{[CLS]} and this was used for classification into real/fake.
...
We chose news which have at least three tweets and sampled three tweets for comment generation.
We prepared 200 sets of a news article and comments for each of real and fake classes and trained the model to generate comments.
...
We converted all alphabets to lowercase letter.
...
We measured the effect of generated comment for classification by comparing classification results with and without the generated comments.
...
"Breaking:" may be also signal of fake news.
