...
We defined a field set of an article and three comments as an instance.
...
\end{equation}
Following Grover's method, we partitioned randomly the document fields into two sets.
We also drop out randomly individual fields to train prediction with probability 10\%, and drop out all but the article body with probability 35\%\cite{NIPS2019_9106}.
The generation model will be trained to minimize the cross-entropy in prediction in one field set followed by the tokens in another set.
Basically our model was constructed by replacing fields in Grover model’s news structure with comments, except for article.
...
In the classification, we removed a comment from each instance and replaced with a generated comment by inference of the trained comment generator from the other fields(a document and two comments).
